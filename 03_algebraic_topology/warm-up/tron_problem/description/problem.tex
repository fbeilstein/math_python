...
\begin{lstlisting}
def wrap_cylinder(x, y, dx, dy, width, height):
    pass
\end{lstlisting}
...
\toprule
\textbf{Argument} & \textbf{Type} & \textbf{Description} \\ \midrule
\lstinline|x, y| & \lstinline|int, int| & The current position coordinates of the light cycle. \\
\lstinline|dx, dy| & \lstinline|int, int| & The current velocity vector (direction of movement). \\
\lstinline|width, height| & \lstinline|int, int| & The dimensions of the grid. Valid coordinates are in $[0, \text{width}-1]$ and $[0, \text{height}-1]$. \\ \midrule
\textbf{Return} & \lstinline|(int, int, int, int) or None| & A tuple \lstinline|(nx, ny, ndx, ndy)| with the calculated new coordinates and potentially flipped velocities, or \lstinline|None| if the cycle crashes into a solid boundary. \\
\bottomrule
\end{tabularx}
\end{center}
