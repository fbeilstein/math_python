...
\lstinline|levels/level_*.py| & Interactive visual sandbox for each problem. \textbf{Can be run standalone} with \lstinline|python levels/level_*.py| for visual debugging. \\
\bottomrule
\end{tabularx}
\end{center}
...
Start with the weak acid equilibrium \ce{HA <=>[k_1][k_2] H+ + A-}.
Write down the equilibrium condition as $K_a = k_1/k_2 = \text{ something}$.
Using definitions $\text{pH} = -\log[\ce{H+}]$ and $\text{p}Ka = -\log K_a$ derive the equation for $\text{pH}$.


Given an initial buffer composition (concentrations of weak acid $C_{acid}$ and its conjugate base $C_{base}$), and a volume $V_{add}$ of a strong titrant (concentration $C_{titrant}$), derive expressions for $[\ce{A-}]$ and $[\ce{HA}]$, note whether the titrant is a strong acid or strong base.
Use Henderson-Hasselbalch to find the pH.
...
\subsubsection*{Implementation part}
\textbf{Implement:} a function \lstinline|analyze_inhibition(S_array, V_no_inh, V_with_inh, I_conc)|. Perform linear regression on both datasets to find the true and apparent $K_m$ and $V_{max}$. Classify the inhibition type based on the slopes/intercepts, and back-calculate $K_i$.

\begin{center}
\renewcommand{\arraystretch}{2.0}
\begin{tabularx}{\textwidth}{r | >{\centering\arraybackslash}X | >{\centering\arraybackslash}X | >{\centering\arraybackslash}X | c | >{\centering\arraybackslash}X}
\toprule
\textbf{Symbol} & $[S]$ & $V_0$ & $V_i$ & $[I]$ & $K_m, V_{max}, \text{type}, K_i$ \\ \midrule
\textbf{Argument} & \lstinline|S_array| & \lstinline|V_no_inh| & \lstinline|V_with_inh| & \lstinline|I_conc| & \lstinline|return| \\ \midrule
\textbf{Type} & \lstinline|np.ndarray[float]| & \lstinline|np.ndarray[float]| & \lstinline|np.ndarray[float]| & \lstinline|float| & \lstinline|tuple[float, float, str, float]| \\
\bottomrule
\end{tabularx}
\end{center}

\textbf{Test:} Press ``L2 Enzyme Inhibition'' button. Check if the type text matches and your $K_i$ matches the true value.
...
\textbf{Type} & \lstinline|list[float]| & \lstinline|float| & \lstinline|float| & \lstinline|list[float]| \\
\bottomrule
\end{tabular}
\end{center}
...
\subsubsection*{Implementation part}
\textbf{Implement:} functions \lstinline|glycolysis_rhs(state, t, Km)|
\begin{center}
\renewcommand{\arraystretch}{2.0}
\begin{tabular}{r | c | c | c | c}
\toprule
...
\end{tabular}
\end{center}
and \lstinline|glycolysis_fixed_point(Km)|.
\vspace{0.2cm}
\begin{center}
\renewcommand{\arraystretch}{2.0}
\begin{tabular}{r | c | c}
