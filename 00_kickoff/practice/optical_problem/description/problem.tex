...
\lstinline|implementation_tasks.py| & \textcolor{red}{Here you write your code.} Contains the core physics and geometry logic (intersections, refraction, and normals) intended for student implementation. You can add your own \lstinline|unittest.TestCase| classes at the bottom and run \lstinline|python implementation_tasks.py| to execute them. \\
...
\end{center}
\textbf{Derive:} The intersection point of the ray and this line $\mathbf{Q}$. \\
...
\end{tabular}
\end{center}
\textbf{Test:} Press "L1 Infinite Line" button in the visual debugger window. Move different points and see how the intersection point is displayed. Fix any bugs if you see inconsistencies, point disappears when it shouldn't or appears in the wrong place.
...
\textbf{Derive:} Conditions that the intersection point $\mathbf{Q}$ of the ray and the line passing through $\mathbf{P}_1$ and $\mathbf{P}_2$ lies on the segment $\overline{P_1P_2}$. \\

\subsubsection*{Implementation part}
\textbf{Implement:} a function \lstinline|intersect_segment(ray_origin, ray_dir, p1, p2)| that returns intersection point $\mathbf{Q}$ as a \lstinline|numpy.array| with two coordinates. If the ray does not intersect the segment, return \lstinline|None|.
...
\end{tabular}
\end{center}
\textbf{Test:} Press "L2 Segment Bounds" button in the visual debugger window. Move different points in visual debugger and ensure your code works appropriately.
...
\textbf{Derive:} The intersection points $\mathbf{Q}_1$ and $\mathbf{Q}_2$ of the ray and the circle. Define them in way $\mathbf{Q}_1$ is \textbf{always closer} to the ray source than $\mathbf{Q}_2$.
...
\end{tabular}
\end{center}
\textbf{Test:} Press "L3 Circle Math" button in the visual debugger window. Move different points in visual debugger and ensure your code works appropriately. \textbf{Note: only the first point from the list is displayed in the debugger.} This is to help you understand whether you return points in the correct order.
...
\textbf{Derive:} The condition that an intersection point $\mathbf{Q}$ lies within the angular sector of the arc (see image below).
...
\end{center}
 that returns \textbf{the nearest} valid intersection point $\mathbf{Q}$ as a \lstinline|numpy.array|. Use the fact that \lstinline|intersect_circle| function returns points in order from closer to the ray source to further. If no point on the arc is hit, return \lstinline|None|.
\begin{center}
\renewcommand{\arraystretch}{2.0}
...
\end{tabular}
\end{center}
\textbf{Test:} Press "L4 Arc Sector" button in the visual debugger window. Move different points in visual debugger and ensure your code works appropriately.
...
\end{tabular}
\end{center}
\textbf{Test:} Press "L5 Line Normals" button in the visual debugger window. Move different points in visual debugger and ensure your code works appropriately. Try reproducing the image in the problem description, ensure that the vector points appropriately.
...
\end{tabular}
\end{center}
\textbf{Test:} Press "L6 Arc Normals" button in the visual debugger window. Move different points in visual debugger and ensure your code works appropriately. Try reproducing the image in the problem description, ensure that the vector points appropriately.
...
\textbf{Derive:} The unit direction vector of the refracted ray $\vec{r}$.
...
\end{tabular}
\end{center}
\textbf{Test:} Press "L7 Refraction" button in the visual debugger window. Move different points in visual debugger and ensure your code works appropriately. See if the light bends. \textbf{Debugger supposes that the light is always going from the air $n_1 = 1$ into the glass $n_2 = 1.5$.} Think if the beam should get closer of further from the normal, check in the visual debugger.
