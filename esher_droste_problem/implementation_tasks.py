...
def backward_step_2_log_polar(W_out):
    return np.exp(W_out)

def backward_step_3_conformal_twist(W_straight, C):
    return W_straight * C

def backward_step_4_exponentiation(Z_out):
    return np.log(Z_out)

def backward_step_5_droste_fold(Z_spiral, m, exact_rotation, Bx, By):
