...
\textbf{Details:} 
\begin{itemize}
    \item \lstinline|chien_search|: Loop over every position $i$ in the message (from $0$ to $msg\_len - 1$). The root candidate is $\alpha^{-i}$, which in our code corresponds to \lstinline|field.exp((field.size - 1 - i) % (field.size - 1))|. Evaluate the locator polynomial at this root. If it evaluates to zero (\lstinline|not err_loc(root)| using our duck-typed \lstinline|__bool__|), append $msg\_len - 1 - i$ to the list of error positions.
