...
\begin{itemize}
    ...
    \item Use \lstinline|try/except| around \lstinline|solve_linear(A, b)|. If it raises an exception (singular matrix), \lstinline|continue| to the next smaller $t$.
    \item If successful, construct and return the polynomial $\Lambda(x) = \Lambda_v x^v + \dots + \Lambda_1 x + 1$. Remember that \lstinline|solve_linear| returns $[\Lambda_v, \dots, \Lambda_1]$. You must append $1$ (\lstinline|field.one|) to the end of this list before passing it to the \lstinline|Polynomial| constructor.
\end{itemize}
