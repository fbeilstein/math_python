...
\begin{itemize}
    ...
    \item Use binary exponentiation (exponentiation by squaring) for \lstinline|__pow__| to guarantee $O(\log n)$ complexity. Ensure $0^0 = 1$. Make sure it works with negative integers. 
    \item \textbf{Hint:} If a division by zero is attempted in \lstinline|__truediv__|, explicitly raise a Python exception: \lstinline|raise ZeroDivisionError()|.
\end{itemize}
