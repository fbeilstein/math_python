...
class Level7Homomorphisms(TabbedLevel):
    ...
    def on_click(self, event):
        ...
        if min_dist_g < threshold:
            ...
            if self.mapping_start_node is not None:
                ...
                contradiction = self.check_consistency(self.partial_phi)
                if contradiction:
                    if old_val is not None:
                        self.partial_phi[start_node] = old_val
                    else:
                        del self.partial_phi[start_node]
                    self.show_error("This arrow creates a contradiction.")
                    self.canvas.get_tk_widget().after(4000, self.update_view)
                    return
    ...
    def update_view(self):
        ...
        is_complete = len(self.partial_phi) == len(self._group_G.elements)
        is_homo = False
        if is_complete:
            is_homo = tasks.is_homomorphism(self._group_G, self.partial_phi)
        ...
        # Update info panel
        lines = [
            f"Source G: order {len(self._group_G.elements)}",
            f"Target H: order {len(self._group_H.elements)}",
        ...
        def sort_key_g(g):
            if g in self.partial_phi:
                target = self.partial_phi[g]
                h_idx = list(self._group_H.elements).index(target)
                return (0, h_idx, str(g))
            return (1, 0, str(g))
            
        g_elements = sorted(self._group_G.elements, key=sort_key_g)
        
        x_g, x_h = 0.2, 0.8
        g_spacing = min(0.8 / max(len(self._group_G.elements), 1), 0.08)
        h_spacing = min(0.8 / max(len(self._group_H.elements), 1), 0.1)
        g_y_start = 0.5 + (len(self._group_G.elements) - 1) * g_spacing / 2
        h_y_start = 0.5 + (len(self._group_H.elements) - 1) * h_spacing / 2

        self._node_pos_G = {}
        self._node_pos_H = {}
        h_colors = {h: colors[i % len(colors)] for i, h in enumerate(self._group_H.elements)}
        h_indices = {h: i for i, h in enumerate(self._group_H.elements)}
        ...
        if self.mapping_start_node is not None:
            subtitle = f"Mapping {str(self.mapping_start_node)} → ... (click a Target)"
